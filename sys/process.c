...
void create_kernel_thread() {
    ...
    //old_rsp = (uint64_t *)(k_pcb->init_kernel - 128);
...
void test_function() {
	...
	switchTask(curr_process, first_process);
    }
}
...
void switchTask(pcb_t *current, pcb_t *next) {
    uint64_t *rsp_addr = (uint64_t *)(next->init_kernel - 128);
//    uint64_t *old_rsp = (uint64_t *)(current->init_kernel - 128);
//    tss.rsp0 = next->init_kernel;

//    __asm__ volatile ("movq %%rsp, %0;" :: "r"(old_rsp));
    __asm__ volatile ("cli"); 

//    __asm__ volatile ("movq %0, %%rax"::"r" (*(old_rsp)));

//    __asm__ volatile ("movq %0, %%rsp;" :: "r"((current->init_kernel)));

//    __asm__ volatile ("pushq %%rax;" :: ); 

//    __asm__ volatile ("movq %%rsp, (%0);" : "=r"(old_rsp));
//    current->init_kernel = (uint64_t)old_rsp;
